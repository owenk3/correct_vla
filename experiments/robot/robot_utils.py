...
from policies.pi05.pi05_utils import get_pi05, get_pi05_action
...
MODEL_IMAGE_SIZES = {
    "openvla": 224,
    "pi05": 256,  # pi05 uses 256x256 for main cameras
}
...
def get_model(cfg: Any, wrap_diffusion_policy_for_droid: bool = False) -> torch.nn.Module:
    ...
    if cfg.model_family == "openvla":
        from policies.openvla.openvla_utils import get_vla
        model = get_vla(cfg)
...
def get_action(
    ...
    action_suppress_mask: Optional[np.ndarray] = None,
    ...
    Args:
        ...
        action_bias: optional (N, 7) additive bias applied to action chunk (pi05 only)
        action_suppress_mask: optional (N, 7) mask multiplied onto VLA actions before bias (pi05 only)
    ...
    with torch.no_grad():
        if cfg.model_family == "openvla":
            from policies.openvla.openvla_utils import get_vla_action
            ...
            action = get_pi05_action(cfg=cfg, model=model, obs=obs, task_label=task_label,
                                     action_bias=action_bias,
                                     action_suppress_mask=action_suppress_mask)
        else:
            raise ValueError(f"Unsupported model family: {cfg.model_family}")
